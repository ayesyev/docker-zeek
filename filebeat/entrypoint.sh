...
geoipInfo(){
  ...
  curl -s --user ${ELK_USER}:${ELK_PASS} -X PUT "${ELASTICSEARCH_HOSTS}/_ingest/pipeline/geoip-info" -H 'Content-Type: application/json' -d'
  {
...
waitForElasticsearch() {
  ...
  i=0;
  while [ $i -le 60 ]; do
    health=$(curl --silent --user ${ELK_USER}:${ELK_PASS} "${ELASTICSEARCH_HOSTS}/_cat/health" | awk '{print $4}')
...
waitForKibana() {
    ...
    while [ $i -le 20 ]; do
        ...
        status=$(curl --silent --user ${ELK_USER}:${ELK_PASS} -XGET ${KIBANA_HOSTS}/api/status | jq -r '.status.overall.state')
